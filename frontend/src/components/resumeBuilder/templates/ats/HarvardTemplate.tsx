...
const Heading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h2 className="border-b border-black pb-0.5 text-[12px] font-bold uppercase tracking-wide text-black">
    {children}
  </h2>
);
...
/**
 * Harvard Resume — modeled on the Harvard OCS format: centered header,
 * underlined section titles, institution bold-left / dates right, role in
 * italics underneath. No color anywhere — this format predates that idea.
 */
export const HarvardTemplate: React.FC<TemplateProps> = ({ resume }) => {
        ...
        <div className="mt-1.5 space-y-2">
          {resume.education.map((edu, i) => (
            <div key={edu._id || i}>
              ...
              <p className="text-[12.5px] italic">{edu.degree}</p>
              {edu.description && <p className="mt-0.5 text-[12.5px]">{edu.description}</p>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-5">
        ...
        <div className="mt-1.5 space-y-3">
          {resume.experience.map((exp, i) => (
            <div key={exp._id || i}>
              <div className="flex items-baseline justify-between text-[12.5px]">
                <p className="font-bold">{exp.company || 'Company'}{exp.location && `, ${exp.location}`}</p>
    ...
    <div className="mx-auto w-full max-w-[720px] bg-white p-10 text-black" style={{ fontFamily: 'Georgia, "Times New Roman", serif' }}>
      <div className="text-center">
        ...
        <p className="mt-1 text-[12px]">
          {[personalInfo.location, personalInfo.phone, personalInfo.email, personalInfo.linkedin].filter(Boolean).join(' • ')}
        </p>
      </div>
