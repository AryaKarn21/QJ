...
/**
 * Premium Professional — a dark header block anchors just the name/title
 * area (not the whole page), serif name paired with sans body copy, and a
 * short accent underline beneath each heading instead of a full-width rule.
 */
export const PremiumProfessionalTemplate: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const Heading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <div>
      ...
      <div className="mt-1 h-[2px] w-8" style={{ backgroundColor: theme.accent }} />
    </div>
  );
  ...
  const Extra: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mt-5">
      ...
      <div className="mt-2 space-y-0.5 text-[12.5px]">{children}</div>
    </section>
  );
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    ...
    skills: () => (
      <section className="mt-5">
        ...
        <p className="mt-2 text-[12.5px]">{skillsAsPlainText(resume.skills)}</p>
      </section>
    ),
    internships: () => (
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] overflow-hidden rounded-lg bg-white shadow-md" style={{ fontFamily: 'Inter, Arial, sans-serif' }}>
      <div className="bg-slate-900 px-10 py-8 text-white">
        <h1 className="text-[28px] font-semibold" style={{ fontFamily: 'Georgia, serif' }}>{personalInfo.fullName || 'Your Name'}</h1>
        <p className="mt-1 text-sm" style={{ color: theme.accentSoft }}>{resume.targetRole || 'Target Role'}</p>
...
export default PremiumProfessionalTemplate;
