...
export const QatarAtsTemplate: React.FC<Props> = ({ resume }) => {
    ...
    <div className="mx-auto min-h-[1050px] w-full max-w-[800px] bg-white text-slate-900 p-8 sm:p-12 font-serif leading-normal shadow-sm print:shadow-none">
      ...
      <div className="border-b border-slate-900 pb-3 text-center">
        ...
        <div className="mt-2 text-xs text-slate-700 flex flex-wrap justify-center gap-x-2 gap-y-0.5">
          ...
          {(countryCVInfo.currentLocation || personalInfo.location) && (
            <span>• Location: {countryCVInfo.currentLocation || personalInfo.location}</span>
          )}
          {countryCVInfo.nationality && <span>• Nationality: {countryCVInfo.nationality}</span>}
          {countryCVInfo.visaStatus && <span>• Status: {countryCVInfo.visaStatus}</span>}
          {countryCVInfo.noticePeriod && <span>• Notice: {countryCVInfo.noticePeriod}</span>}
      ...
      {resume.skills && resume.skills.length > 0 && (
        <section className="mt-4">
          ...
          <p className="text-xs text-slate-800">{resume.skills.map((s) => s.name).join(' • ')}</p>
        </section>
      )}
          ...
          <div className="text-xs text-slate-800 space-y-1">
            ...
            {countryCVInfo.memberships && countryCVInfo.memberships.length > 0 && (
              <p>
                ...
                {countryCVInfo.memberships.map((m) => `${m.organization} (${m.membershipType})`).join(', ')}
              </p>
            )}
