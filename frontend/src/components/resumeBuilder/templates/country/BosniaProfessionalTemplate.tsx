            ...
            {(() => {
              ...
              return (
                <div className="space-y-1 text-[9.5px] sm:text-[10px] text-slate-700 leading-relaxed">
                  {categoriesSet.size > 1 ? (
                    Object.entries(categoryGroups).map(([cat, items]) => (
                      <React.Fragment key={cat}>
                        {renderSkillRow(items, cat === 'General' ? undefined : `${cat} Vještine`)}
                      </React.Fragment>
                    ))
                  ) : (
                  ...
                  {digitalSkillItems.length > 0 && renderSkillRow(digitalSkillItems, 'Digitalne vještine')}
                  {softwareSkillItems.length > 0 && renderSkillRow(softwareSkillItems, 'Softverske vještine')}

                  {otherSkillsRaw && (
                    otherSkillItems.length > 1 ? (
                      renderSkillRow(otherSkillItems, 'Ostale vještine')
                    ) : (
                      <div className="text-[9.5px] sm:text-[10px] text-slate-700 leading-relaxed break-inside-avoid">
                        <span className="font-bold text-slate-900 mr-1.5 inline-block select-none">
                          Ostale vještine:
