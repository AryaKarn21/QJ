...
/**
 * Data Analyst — the top 3 achievements get pulled into a highlight strip
 * of stat-style boxes right under the header (skipped entirely if there
 * are none, or if the Achievements section is hidden, rather than showing
 * empty boxes), since quantified impact is what this role is evaluated on.
 */
export const DataAnalystTemplate: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const remainingAchievements = achievementsVisible ? resume.achievements.slice(3) : [];
  ...
  // `achievements` only renders the "remaining" (4th+) items here, since
  // the first 3 are pulled into the highlight strip above the loop.
    ...
    achievements: () =>
      remainingAchievements.length > 0 ? (
        <Extra title="Achievements">
          {remainingAchievements.map((a, i) => <p key={a._id || i}>{a.title} {a.year && `(${a.year})`} {a.link && <ResumeLink href={a.link} label="View Proof" color={theme.accent} />}</p>)}
        </Extra>
      ) : null,
    publications: () => (
    ...
    <div className="mx-auto w-full max-w-[720px] bg-white p-10 text-slate-800" style={{ fontFamily: 'Inter, Arial, sans-serif' }}>
      ...
      {highlights.length > 0 && (
        <div className="mt-4 grid grid-cols-3 gap-2">
          {highlights.map((a, i) => (
            <div key={a._id || i} className="rounded-lg p-2.5" style={{ backgroundColor: theme.accentSoft }}>
              <p className="text-[11.5px] font-semibold" style={{ color: theme.accent }}>{a.title}</p>
              {a.year && <p className="text-[10px] text-slate-500">{a.year}</p>}
            </div>
          ))}
        </div>
      )}
