...
const Heading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h2 className="font-mono text-[11px] font-bold uppercase tracking-wide text-slate-700">
...
/**
 * Cyber Security — a dark terminal-style header (monospace, prompt-style
 * name) sets this apart from every other template in the set, and
 * certifications are promoted near the top since they're often mandatory
 * screening criteria for this field.
 */
export const CyberSecurityTemplate: React.FC<TemplateProps> = ({ resume }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] bg-white text-slate-800" style={{ fontFamily: 'Inter, Arial, sans-serif' }}>
      <div className="bg-slate-950 px-10 py-8 font-mono text-white">
        <p className="text-[11px]" style={{ color: GREEN }}>~ whoami</p>
        <h1 className="mt-1 text-2xl font-bold">{personalInfo.fullName || 'Your Name'}</h1>
        <p className="mt-1 text-sm text-slate-300">{resume.targetRole || 'Target Role'}</p>
