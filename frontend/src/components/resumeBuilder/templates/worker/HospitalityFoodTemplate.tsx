...
const SectionHeading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="mb-2.5">
    <h2 className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#7b3f00]">{children}</h2>
    <div className="flex gap-1 mt-1">
      <div className="h-[2px] w-8 bg-[#7b3f00]" />
      <div className="h-[2px] flex-1 bg-[#e8d5b7]" />
    </div>
  </div>
);

/**
 * Hospitality & Food Service — warm cream and brown tones, highlights customer
 * service, food safety, housekeeping, and hospitality skills. Clean and welcoming.
 */
export const HospitalityFoodTemplate: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => (
      <section className="mb-5">
        ...
        <p className="text-[12.5px] leading-relaxed italic text-[#444]">{resume.summary}</p>
      </section>
    ),
    skills: () => (
      <section className="mb-5">
        <SectionHeading>Skills &amp; Service Areas</SectionHeading>
        <div className="flex flex-wrap gap-2">
          {resume.skills.map((s, i) => (
            <span key={s._id || i} className="rounded-full bg-[#f5deb3] px-3 py-1 text-[11.5px] font-semibold text-[#7b3f00]">
              {s.name}
            </span>
          ))}
        </div>
          ...
          {resume.experience.map((exp, i) => (
            <div key={exp._id || i}>
              <div className="flex items-baseline justify-between">
                <p className="text-[13px] font-bold text-[#7b3f00]">{exp.role || 'Role'}</p>
              ...
              {exp.description && (
                <ul className="mt-1.5 list-disc pl-4 space-y-0.5">
                  {toBulletLines(exp.description).map((line, li) => (
                    <li key={li} className="text-[12px] leading-relaxed text-[#444]">{line}</li>
                  ))}
                </ul>
              )}
      ...
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.internships.map((it, i) => (
            ...
            {it.link && <ResumeLink href={it.link} label="Company Link" className="mt-0.5 inline-block" />}
            </p>
          ))}
        </div>
      </section>
      ...
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.volunteering.map((v, i) => (
            ...
            {v.link && <ResumeLink href={v.link} label="Organization Link" className="mt-0.5 inline-block" />}
            </p>
          ))}
        </div>
      </section>
    ...
    certifications: () => (
      <section className="mb-5">
        <SectionHeading>Certifications &amp; Food Safety</SectionHeading>
        <div className="space-y-1">
      ...
      <section className="mb-5">
        ...
        {resume.education.map((edu, i) => (
          <div key={edu._id || i} className="flex items-baseline justify-between text-[12.5px] mb-1">
            ...
            <p className="text-[11px] text-[#888] italic">{edu.startDate}–{edu.endDate}</p>
    ...
    publications: () => (
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.publications.map((p, i) => <p key={p._id || i} className="text-[12.5px]">{p.title}, {p.publisher} {p.link && <ResumeLink href={p.link} label="View Publication" />}</p>)}
        </div>
      </section>
    ),
    trainings: () => (
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.trainings.map((t, i) => <p key={t._id || i} className="text-[12.5px]">{t.title}, {t.provider} {t.link && <ResumeLink href={t.link} label="View Course" />}</p>)}
        </div>
      </section>
    ),
    scholarships: () => (
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.scholarships.map((s, i) => <p key={s._id || i} className="text-[12.5px]">{s.title}, {s.institution} {s.link && <ResumeLink href={s.link} label="View Award" />}</p>)}
        </div>
      </section>
    ),
    positionsOfResponsibility: () => (
      <section className="mb-5">
        ...
        <div className="space-y-1">
          {resume.positionsOfResponsibility.map((p, i) => <p key={p._id || i} className="text-[12.5px]">{p.title}, {p.organization} {p.link && <ResumeLink href={p.link} label="Organization Link" />}</p>)}
        </div>
      </section>
    ),
    languages: () => (
    ...
    references: () => (
      <section>
        ...
        {resume.references.map((r, i) => (
          ...
          {r.link && <ResumeLink href={r.link} label="Profile" className="mt-0.5 inline-block" />}
          </p>
        ))}
      </section>
    ),
  ...
  return (
    <div
      className="mx-auto w-full max-w-[720px] bg-[#fffdf8] text-[#2c2c2c]"
      style={{ fontFamily: 'Georgia, serif' }}
    >
      {/* Header */}
      <div className="bg-[#7b3f00] text-white px-10 py-7">
        <h1 className="text-[27px] font-bold tracking-wide">
          {personalInfo.fullName || 'Your Name'}
        </h1>
        {resume.targetRole && (
          <p className="text-[13px] mt-0.5 text-[#f5deb3] font-medium italic">{resume.targetRole}</p>
        )}
        <div className="mt-3 text-[11px] text-[#f5deb3] flex flex-wrap gap-x-5 gap-y-1">
          ...
          if (id.startsWith('custom:')) {
            ...
            return (
              <section key={id} className="mb-5">
                ...
                <p className="whitespace-pre-wrap text-[12.5px] leading-relaxed italic text-[#444]">{custom.content}</p>
...
export default HospitalityFoodTemplate;
