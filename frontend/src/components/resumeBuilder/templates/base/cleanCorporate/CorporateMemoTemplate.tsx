...
/**
 * Clean Corporate — Memo Style. No photo. A light gray banner header reads
 * like a company letterhead, and every section carries a left accent bar —
 * structured like an internal corporate memo rather than a personal résumé.
 */
export const CorporateMemoTemplate: React.FC<TemplateProps> = ({ resume }) => {
  const theme = getTheme(resume.theme, resume.fontFamily);
  const { personalInfo } = resume;

  const Heading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h2 className="border-l-[3px] pl-2.5 text-[11.5px] font-bold uppercase tracking-wide text-slate-700" style={{ borderColor: theme.accent }}>
      {children}
    </h2>
  );
  const Extra: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
    <section className="mt-4">
      ...
      <div className="mt-1.5 space-y-0.5 pl-2.5 text-[12.5px]">{children}</div>
    </section>
  );
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => (
      <section className="mt-4">
        ...
        <p className="mt-1.5 pl-2.5 text-[12.5px] leading-relaxed">{resume.summary}</p>
      </section>
    ),
    experience: () => (
      <section className="mt-4">
        <Heading>Experience</Heading>
        <div className="mt-1.5 space-y-3 pl-2.5">
    ...
    projects: () => (
      <section className="mt-4">
        ...
        <div className="mt-1.5 space-y-1.5 pl-2.5">
    ...
    skills: () => (
      <section className="mt-4">
        ...
        <p className="mt-1.5 pl-2.5 text-[12.5px]">{skillsAsPlainText(resume.skills)}</p>
      </section>
    ),
    internships: () => (
  ...
  return (
    <div className="mx-auto w-full max-w-[720px] bg-white text-slate-800" style={{ fontFamily: theme.fontBody }}>
      <div className="bg-slate-100 px-10 py-6">
          ...
          if (id.startsWith('custom:')) {
            ...
            return (
              <section key={id} className="mt-4">
                ...
                <p className="mt-1.5 pl-2.5 whitespace-pre-wrap text-[12.5px] leading-relaxed">{custom.content}</p>
              </section>
            );
          }
...
export default CorporateMemoTemplate;
