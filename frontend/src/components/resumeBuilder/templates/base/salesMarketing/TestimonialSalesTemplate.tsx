...
/**
 * Sales/Marketing — Testimonial. No photo. The summary renders as a large
 * pull-quote (oversized quotation mark, italic), styled like a client
 * testimonial pulled to the top — a persuasive, marketing-forward opener
 * distinct from Quota Metrics' data-first approach.
 */
export const TestimonialSalesTemplate: React.FC<TemplateProps> = ({ resume }) => {
  ...
  const sectionRenderers: Record<string, () => React.ReactNode> = {
    summary: () => (
      <section className="mt-5 rounded-lg p-4" style={{ backgroundColor: theme.accentSoft }}>
        <p className="text-[14px] font-medium italic leading-relaxed" style={{ color: theme.accent }}>
          <span className="mr-1 text-2xl leading-none">&ldquo;</span>{resume.summary}
        </p>
      </section>
    ),
    experience: () => (
...
export default TestimonialSalesTemplate;
